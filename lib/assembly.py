...
def create_debug_header(file, address):
    ...
    debug_header = debug_header.replace('\\', '\\\\')
...
def insert_bytes_into_codes_file(
    codes_file_location, bytes, address, include_type=True
):
...
# code type is the enum declared above
def insert_immediate_exec_bytes_into_codes_file(
    codes_file_location, bytes, include_type=True
):
    # now inject the code into the dol
    with open(codes_file_location, "ab") as dol_writer:
        if include_type:
...
# code type is the enum declared above
def insert_code_with_explicit_return_address_into_codes_file(
    codes_file_location, file, address, return_address
):
...
def insert_assembly_into_codes_file(
    codes_file_location,
    file,
    address,
    jinja_replacement_dict,
    include_type=True,
    ...
    if immediate_exec:
        insert_immediate_exec_bytes_into_codes_file(
            codes_file_location, bytes, include_type
        )
    else:
        insert_bytes_into_codes_file(codes_file_location, bytes, address, include_type)


def insert_level_assembly_into_codes_file(
    dol, codes_file_location, data, address, level_index, jinja_replacement_dict
):
    level_switch_code = f"""
 ####### LEVEL BYPASS CODES ######
 ...
 ####### LEVEL BYPASS CODES ######
  ...
  """
    ...
    data = (
        create_debug_header(file, address)
        + level_switch_code
        + data
...
def insert_player_inventory_into_codes_file(
    ...
    for i in range(1, 58):
      ...
      """
            ...
            battery_count = (
                level_invent_map["battery_count"]
                if "battery_count" in level_invent_map
                else 3
            )
            default_primary_slot = (
                level_invent_map["default_primary_slot"]
                if "default_primary_slot" in level_invent_map
                else 1
            )
            default_secondary_slot = (
                level_invent_map["default_secondary_slot"]
                if "default_secondary_slot" in level_invent_map
                else 0
            )
      ...
      """
        ...
        """
            ...
            washer_count = (
                level_invent_map["washer_count"]
                if "washer_count" in level_invent_map
                else 0
            )
            code_string += f"""
         or r3, r31, r31 ; inventory pointer
      ...
      """
            ...
            chip_count = (
                level_invent_map["chip_count"]
                if "chip_count" in level_invent_map
                else 0
            )
            code_string += f"""
         or r3, r31, r31 ; inventory pointer
      ...
      """
            ...
            secret_chip_count = (
                level_invent_map["secret_chip_count"]
                if "secret_chip_count" in level_invent_map
                else 0
            )
            code_string += f"""
         or r3, r31, r31 ; inventory pointer
      ...
      """
            ...
            arm_servo_count = (
                level_invent_map["arm_servo_count"]
                if "arm_servo_count" in level_invent_map
                else 1
            )
            code_string += f"""
         or r3, r31, r31 ; inventory pointer
      ...
      """
            ...
            det_pack_count = (
                level_invent_map["det_pack_count"]
                if "det_pack_count" in level_invent_map
                else 0
            )
            code_string += f"""
         or r3, r31, r31 ; inventory pointer
      ...
      """
            ...
            goff_part_count = (
                level_invent_map["goff_part_count"]
                if "goff_part_count" in level_invent_map
                else 0
            )
            code_string += f"""
         or r3, r31, r31 ; inventory pointer
...
def insert_player_spawn_into_codes_file(codes_file_location, level_bot_map):
    ...
    for i in range(len(level_bot_map)):
        ...
        if lower_bot_name == "random":
            ...
            code_string = lib.assembly_codes.BOT_NAME_DICT[lower_bot_name].replace(
                "LABEL", level_prefix
            )
        else:
            raise ValueException(
                f"Unknown player bot type: {level_bot_map[i]} on level {i + 1}"
            )
        ...
        for j in range(25):
            if f"{level_prefix}LABEL_{j}" in code_string:
                code_string = code_string.replace(
                    f"{level_prefix}LABEL_{j}",
                    f"{level_prefix}LABEL_{internal_label_count}",
                )
