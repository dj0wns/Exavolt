...
from .assembly import (
    ...
    insert_level_assembly_into_codes_file,
)
...
def insert_mod(
    ...
    for entry in data["scratch_memory"]:
        if not entry["global"]:
            ...
            add_entry_to_dict(
                entry, local_scratch_memory_replacement_dict, memory_offset
            )
    ...
    for level in data["levels"]:
        if level["type"] == LEVEL_TYPES[0] and (
            ...
            if "projector_range_adjustment" in level:
                level_list[level_index].projector_offsets = level[
                    "projector_range_adjustment"
                ]
            ...
            new_level.secret_chips = (
                level["secret_chip_count"] if "secret_chip_count" in level else 0
            )
            new_level.time_to_beat = (
                level["speed_chip_time"] if "speed_chip_time" in level else 0
            )

            if level["type"] == LEVEL_TYPES[0]:  # campaign
                insert_level_list.append(
                    [level["index"] if "index" in level else -1, new_level]
                )
            else:
            ...
            if os.path.splitext(info.filename)[1].lower() == ".mst":
                ...
                for filename in path.iterdir():
                    new_filename = os.path.join(
                        insert_file_dir.name, os.path.basename(filename)
                    )
                ...
                for level_assembly_file in level_assembly_files:
                    ...
                    if os.path.basename(info.filename) == level_assembly_file["file"]:
                        ...
                        with open(file_path, "r") as file:
                            level.assembly_files.append(
                                (
                                    level_assembly_file["injection_location"],
                                    file.read(),
                ...
                if os.path.basename(info.filename) in assembly_files.keys():
                    ...
                    print(
                        f"Extracting {info.filename} to {os.path.join(iso_dir, 'root')}"
                    )
                    ...
                    file_path = os.path.join(tmpdirname.name, info.filename)
                    new_filename = os.path.join(
                        iso_dir, "root", "files", os.path.basename(info.filename)
                    )
                    print(file_path, new_filename)
                    shutil.move(file_path, new_filename)
