...
def parse_dol_table(dol, debug=False):
    ...
    for i in range(18):
        file_offsets.append(
            int.from_bytes(dol_reader.read(4), byteorder="big", signed=False)
        )
    for i in range(18):
        memory_offsets.append(
            int.from_bytes(dol_reader.read(4), byteorder="big", signed=False)
        )
    for i in range(18):
    ...
    if debug:
        ...
        print(
            f"bss addr: {bss_address}({hex(bss_address)}), size: {bss_size}({hex(bss_size)})"
        )
        print(f"entry_point: {entry_point}({hex(entry_point)})")
        for i in range(len(file_offsets)):
            print(
                f"{offset_names[i]}: {file_offsets[i]}({hex(file_offsets[i])}) -> {memory_offsets[i]}({hex(memory_offsets[i])}), {section_sizes[i]}({hex(section_sizes[i])}) bytes, end: {section_sizes[i] + file_offsets[i]}({hex(section_sizes[i] + file_offsets[i])}) -> {section_sizes[i] + memory_offsets[i]}({hex(section_sizes[i] + memory_offsets[i])})"
            )
...
def inject_assembly(dol, file, address):
    ...
    jump_instruction = get_jump_instruction(
        final_address, address + 4
    )  # dont forget to jump after the caller!
    bytes += jump_instruction.to_bytes(4, byteorder="big")
    print(
        f"Injection starts at {hex(next_code_injection_virtual_offset)}, and ends at {hex(final_address)} and is {len(bytes)} bytes long. It makes a jump of length {jump_delta}, ({hex(jump_delta)}) with opcode {hex(jump_instruction)}"
    )
    ...
    with open(dol, "r+b") as dol_writer:
        ...
        jump_instruction = get_jump_instruction(
            address, next_code_injection_virtual_offset
        )
        print(
            f"Injecting jump to inserted assembly at {hex(address)} to {hex(next_code_injection_virtual_offset)} with opcode {hex(jump_instruction)}"
        )
        ...
        print(
            f"replacing {hex(original)} with {jump_bytes.hex()} at {hex(address)}:{hex(file_address)}"
        )
            ...
            if state == 0x4:
                ...
                with open(dol, "r+b") as dol_reader:
                    ...
                    original = int.from_bytes(
                        dol_reader.read(4), byteorder="big", signed=False
                    )
                    dol_reader.seek(file_address)
                ...
                print(
                    f"replacing {hex(original)} with {code.hex()}) at {hex(address)}:{hex(file_address)}"
                )
