...
def default_scratch_memory_entries(memory_offset):
    ...
    default_scratch_memory_entries = [
        {"name": "SAVE_FILE_POINTER", "size": 4},
        {"name": "LEVEL_LIST", "size": 18000},  # space for 400 levels
        {
            "name": "MODIFIED_NAME_BUFFER",
            "size": 32,
        },  # Used for save files to make the secondary file unique - each characters is 2 bytes!
        {
            "name": "LEVEL_ARRAY_OFFSET",
            "size": 4,
        },  # Offset in memory to the new level array! Hacky because it can
